...
void transfer() {
    ...
    for (int i = 0; i < 10; i++) {	
        int izvor = rand()%UKUPNO_RACUNA; //slucajno izaberemo prvi racun
        int cilj = (izvor+5)%UKUPNO_RACUNA; //slucajno izaberemo drugi racun (ne moze biti isti kao prvi)
        int iznos = rand()%10 + 1; //slucajno izaberemo iznos
		...
		if ((r.staro - r.novo) != iznos) 
            cout << "Prebaceno " << iznos << " sa racuna " << izvor << " na racun " //ova poruka se ne sme ispisati ako program radi ispravno
