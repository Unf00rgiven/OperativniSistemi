...
public:
	...
	auto it = find_if(free_list.begin(), free_list.end(),
    ...
    {
	unique_lock<mutex> l(m);
	if (segments.find(segment) == segments.end()) return;

	Fragment f = segments[segment];
	segments.erase(segment);

	Fragment ff = insert_free_fragment(f);
	diagnostics.deallocateMessage(pid, f.loc, f.len, ff.loc, ff.len);
	...
	if (segments.find(segment) == segments.end()
	    || address >= segments[segment].len) return 0;

	diagnostics.readMessage(pid, segment, address);
	return buffer[segments[segment].loc + address];
    }
	...
	unique_lock<mutex> l(m);
	if (segments.find(segment) == segments.end()
	    || address >= segments[segment].len) return;

	diagnostics.writeMessage(pid, segment, address);
	buffer[segments[segment].loc + address] = value;
    }

    void wait(int pid, uint32_t size)
    {
	unique_lock<mutex> l(m);
...
public:
	...
	long first_segment = memory.allocate(pid, size);
	segments[first_segment] = size;
	...
	while (running)
	{
		...
		while (running && !(segment = memory.allocate(pid, size)))
		    memory.wait(pid, size);
		if (segment) segments[segment] = size;
	    }
	    else if (rnd < 10)
	    {
		map<long, uint32_t>::iterator it;
		...
		if (segments.size() > 1)
		{
			...
			if (it->first != first_segment) break;
		    }
		...
		advance(it, rand() % segments.size());
		memory.read(pid, it->first, rand() % it->second);
	    }
	    else
	    {
		auto it = segments.begin();
		advance(it, rand() % segments.size());
		memory.write(pid, it->first, rand() % it->second, rand() & 0xFF);
	    }
...
int main()
{
    ...
    Process p[PROCESSES] { Process(m, 1), Process(m, 2), Process(m, 3), Process(m, 4) };
    thread t[PROCESSES];
